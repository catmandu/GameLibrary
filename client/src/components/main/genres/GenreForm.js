...
const GenreForm = () => {
  const { AddGenre, UpdateGenre, RemoveGenre, GetGenres, current, genres } =
  ...
  const OnSubmit = (e) => {
    ...
    if (name === '') {
      ...
      SetConfirm(
        `${actionTitle} genre`,
        `Are you sure you want to ${actionTitle.toLocaleLowerCase()} the current genre?`,
        async () => {
          const Action =
            current === null
              ? AddGenre
