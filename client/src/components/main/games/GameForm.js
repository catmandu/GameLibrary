...
const GameForm = () => {
  const { AddGame, UpdateGame, RemoveGame, GetGames, current, games } =
  ...
  const { platforms: availablePlatforms, GetPlatforms } =
  ...
  const OnSubmit = async (e) => {
    ...
    const errorMessage = fieldsIncomplete
      ? 'Fill all the fields'
      : genresUnassigned
    ...
    if (fieldsIncomplete || genresUnassigned || platformsUnassigned) {
      ...
      SetConfirm(
        `${actionTitle} game`,
        `Are you sure you want to ${actionTitle.toLocaleLowerCase()} the current game?`,
        async () => {
          const Action =
            current === null
              ? AddGame
  ...
  return (
    <form onSubmit={OnSubmit}>
      ...
      <TextBox
        title='Art Cover'
        placeholder="url to game's art cover"
        name='cover'
        deleteState={deleteSelected}
        state={game}
        SetState={SetGame}
      ...
      {game.cover !== undefined && (
        <div className='container'>
          <img alt='game cover' src={game.cover} style={{ width: '150px' }} />
        </div>
      )}
