...
const PublisherProvider = (props) => {
  ...
  const UpdatePublisher = async (publisher) => {
    try {
      const res = await axios.put(
        `/gamelibrary/api/publishers/${publisher._id}`,
        publisher
      );
  ...
  const SetCurrentPublisher = (currentPublisher) =>
    Dispatch({ type: SET_CURRENT_PUBLISHER, payload: currentPublisher });

  // Set loading
  const SetLoading = () => Dispatch({ type: SET_LOADING });

  const SetCurrentPage = (selectedPage) =>
